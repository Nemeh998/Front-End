...
import IO2 from "./componentsadmain/IO2";
import Form2 from "./componentsadmain/Form2";
...
class App extends React.Component {
  ...
  //========================================
  // delete sectors
  deleteSectors = async (sectorsId, mainSector,idx) => {
    console.log('first')
    console.log(sectorsId, mainSector)

    const serverUrl = process.env.REACT_APP_SERVER;

    const url = await axios.delete(`${serverUrl}/deletSectorsDataHendler?sectorid=${sectorsId}&mainSector=${mainSector}&idx=${idx}`);
    this.setState({
      data: url.data,
